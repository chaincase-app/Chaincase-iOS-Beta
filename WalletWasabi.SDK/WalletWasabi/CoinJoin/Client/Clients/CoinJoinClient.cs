			...
			try
			{
				using (BenchmarkLogger.Measure(operationName: "TryProcessStatusAsync"))
				{
					...
					using (await MixLock.LockAsync().ConfigureAwait(false))
					{
						...
						{
							ClientRound roundRegistered =
								State.GetSingleOrDefaultRound(DelayedRoundRegistration.AliceClient.RoundId);
						...
						using (await MixLock.LockAsync().ConfigureAwait(false))
						{
							...
							if (registrableRound is { })
							{
								...
								if (CoordinatorFeepercentToCheck is { } &&
								    registrableRound.State.CoordinatorFeePercent > CoordinatorFeepercentToCheck)
								{
							...
							double delaySeconds =
								...
								0.8167; // delay the response to defend timing attack privacy.
							...
							{
								if (inputRegistrableRound
									.Registration is null) // If did not register already, check what can we register.
								{
